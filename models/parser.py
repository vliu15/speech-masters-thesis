...
class CMUDictParser(object):
    ...
    def __init__(self, cmu_dict_path=None):
        ...
        self._replace = [
            (re.compile("\\b%s\\b" % x[0], re.IGNORECASE), x[1]) for x in [
                ("_", "underscore"),
                ("src", "source"),
                ("dll", "d l l"),
                ("btw", "by the way"),
                ("http", "h t t p"),
                ("www", "w w w"),
                ("c\+\+", "c plus plus"),  # pylint: disable=W1401
            ]
        ]
    ...
    def expand_numbers(self, text):
        ...
        text = " ".join(re.split("(\d+)", text))  # pylint: disable=W1401
